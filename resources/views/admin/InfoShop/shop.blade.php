...
@section("content")
                    ...
                    <div class="table-responsive">
                        <table id="example2" class="table table-striped table-bordered">
                            ...
                            @foreach($data as $list)
                                <tr>
                                    <td>{{$list->name}}</td>
                                    <td><img src="{{asset($list->logo)}}" width="100px" height="100px"></td>
                                    <td>{{$list->phone_number}}</td>
                                    <td>{{$list->email}}</td>
                                    <td>{{$list->address}}</td>
                                    <td>
                                        <button type="button"
                                                onclick="saveData('{{$list->id}}','{{$list->name}}','{{$list->logo}}',
                                                '{{$list->phone_number}}','{{$list->email}}','{{$list->address}}',)"
                                                class="btn btn-outline-info px-5 radius-30" data-bs-toggle="modal"
                                                data-bs-target="#exampleModal">Update
                                        </button>
                                        <button onclick="deleteData('{{$list->id}}','infomation_shops')" class="btn btn-outline-danger px-5 radius-30">Delete</button>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
    ...
    <script>
        function saveData(id, name, logo,phone_number,email,address) {
            $('#enter_id').val(id);
            $('#enter_text').val(name);
